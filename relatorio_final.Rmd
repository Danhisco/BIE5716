...
## Cenários explorados de assimetria competitiva e tamanho inicial da invasora
  
<p>Apenas o parâmetro que descreve a probabilidade de exclusão de uma espécie pela outra por colisão (parâmetros $C_n$ e $C_m$, tabela 1) variou entre simulações. Os demais parâmetros populacionais foram iguais: $\beta =$ `r format(0.7/365.25,scientific = TRUE,digits = 4)`, $\delta =$ `r format(0.69/365.25,scientific = TRUE,digits = 4)`, $\gamma =$ `r format(2*(0.7/365.25-0.69/365.25)/50,scientific = TRUE,digits = 4)`. Os parâmetros de competição interespecífica ($C_n$ e $C_m$) variam entre $\gamma/2$ e $\gamma$, tal que a assimetria competitiva, definido por $2(C_n - C_m)/ \gamma$, variou entre -1 e 1 (figura 1). O tamanho inicial da espécie invasora variou entre 1% e 100% da capacidade de suporte K = 50 indivíduos (Figura 1). Foram simulados 11 cenários de tamanho inicial da invasora e 11 cenários de assimetria competitiva, totalizando 121 cenários, cada um com 100 réplicas.</p> 
...
df_plot <- df_sim0 |> filter(C_nm %in% c(1,0,-1) & M0==2) |>  select(-M0)
# C_n = df_plot$C_n[3]
# C_m = df_plot$C_n[1]
# x=0
# equação 4a
f_IsoResidente <- function(x,C_n) (df_plot$beta[1] - df_plot$delta[1]) / C_n - x * df_plot$gamma[1]/(2*C_n)
f_IsoInvasora <- function(x,C_m) 2*(df_plot$beta[1] - df_plot$delta[1])/df_plot$gamma[1] - 2*C_m*x/df_plot$gamma[1]
# curve(f_IsoResidente(x,C_n=df_plot$C_n[1]),from = 0,to = 50)
df_plot <- left_join(x=data.frame(x=rep(0:50,3),id = rep(c(1,6,11),each=51)),
                      y=df_sim0 |> filter(C_nm %in% c(1,0,-1) & M0==2) |>  select(-M0),
                      by="id") |> 
  select(-id,-k) |> 
  mutate(iso_Residente = f_IsoResidente(x=x,C_n=C_n),
         iso_Invasora = f_IsoInvasora(x=x,C_m=C_m))
l_p[[2]] <- df_plot |> 
  pivot_longer(cols = c("iso_Residente", "iso_Invasora"),
               names_to = "species",
               values_to = "N") |> 
  mutate(C_nm = factor(C_nm,levels = c(1,0,-1))) |> 
  ggplot(aes(x=x,y=N,color=species)) +
  geom_line(alpha=0.5) + 
  ylim(0,50) + 
  xlim(0,50) +
  coord_cartesian(expand = FALSE) +
  scale_color_manual(labels = c("residente", "Invasora"), 
                     values = c("blue", "red")) +
  labs(x="residente",y="invasora",color="",title="b) Isolinhas Campo Médio") +
  theme_bw() + theme(legend.position="right") +
  facet_wrap(~C_nm,ncol=1)
grid.arrange(grobs=l_p,ncol=2,layout_matrix=matrix(c(1,1,1,2,2),nrow=1))
```
...
df_sim0 <- cbind(df_sim0,
                 expand.grid(M0 = floor(df_sim0$k * c(0.05,seq(0.1,1,by=0.1))),
                             C_n = (df_sim0$gamma/2) * seq(1,2,length.out=6))) |> 
  mutate(C_m = gamma/2)
...
```{r f 1sppSLV, echo=FALSE,eval=FALSE}
...
```{r simulacao tempo de extincao das invasoras,eval=FALSE,echo=FALSE}
...
```
```{r análise dos dados da pasta single_species,echo=FALSE,eval=FALSE}
# df <- df_sim[1,]
f_resultados <- function(df){
  df_ab <- read_csv(df$path_) |> as.data.frame()
  df$t_single <- df_ab[nrow(df_ab),"t"]
  df <- mutate(df,t_total = t + t_single)
}
registerDoMC(3)
df_1sp <- adply(df_sim,1,f_resultados,.parallel = TRUE)
# df dos resultados específicos
## df_TEI2spp.csv
df_1 <- df_1sp |> select(id,.n,invasion,t_total) |> 
  rename(TE_m = t_total)
df_2 <- df_invasoras |> filter(invasion==0) |> select(-m) |> rename(TE_m = t)
df_1 <- rbind(df_1,df_2)
df_2 <- df_replicas |> select(id,.n,M0,C_nm)
df_TEI2spp <- inner_join(df_1,
                         df_2,
                         by = c("id",".n")) |> 
  arrange(id,.n)
write_csv(df_TEI2spp,file="resultados/df_TEI2spp.csv")
## df_TE1sp.csv
df_TE1sp <- df_1sp |> select(id,.n,m,t_single) |> rename(M0 = m, TE = t_single) |> arrange(id,.n)
write_csv(df_TE1sp,file="resultados/df_TE1sp.csv")
```
...
df_resultados |> 
  ...
  labs(fill="",x="M0, initial pop. size of invader (%K)",y="Cn - Cm / gamma") +
  coord_cartesian(expand = FALSE) +
...
__tabela A1__ seleção de GLMs Binomiais para descrever a probabilidade de substituição
...
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
l_p <- list()
l_p[[1]] <- df_1sp |> 
...
```
```{r analise dos dados de tempo de extincao conjunto M0 menor do que 60, fig.height=5,fig.width=8}
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=60 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
l_p <- list()
...
l_p[[4]] <- df_1sp |> 
  ...
  geom_smooth(method = "loess",formula = "y~x") +
  facet_wrap(~label)
grid.arrange(grobs=l_p,ncol=2,top="Conjunto Parcial: 60 >= tamanho inicial > 1")
```
...
# modelos
l_md <- list()
l_md[[1]] <- lm(log_TE ~ M0 + I(M0^2),data = df_1sp)
l_md[[2]] <- lm(log_TE ~ M0,data = df_1sp)
l_md[[3]] <- lm(log_TE ~ log_M0 + I(log_M0^2),data = df_1sp)
l_md[[4]] <- lm(log_TE ~ log_M0,data = df_1sp)
l_md[[5]] <- lm(log_TE ~ 1,data = df_1sp)
...
```{r graficos diagnostico padrao log lm TE 1sp}
par(mfrow=c(2,2))
plot(l_md[["log(M0)^2"]])
```
...
```{r lm log(TE) logM0 1sp modelo medio}
# dados
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=100 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
# modelo referência
md_ref <- lm(log_TE ~ log_M0 + I(log_M0^2),
             data = df_1sp, na.action = "na.fail")
# todas as combinações do modelo cheio
registerDoMC(3)
l_dredge_nInvasions <- llply(dredge(md_ref,trace = FALSE,evaluate=FALSE), eval,.parallel = TRUE)
# Model Averaging for the observed data
...
df_md <- df_1sp
# interval = 'confidence' ainda não é usado, apenas para uma avaliação visual
df_md$log_TE_predict <- predict(mdAvg_MuMIn,type="response")
# Gráfico
colors <- c("loess" = "blue", "1 : 1" = "black")
...
__Tabela A7__ GLM Gamma(log): Tabela seleção do modelo referência para descrever o log(Tempo de Extinção).  

```{r ajuste modelo cheio GLM Gamma(log) TE 1sp,eval=TRUE,echo=FALSE}
# dados
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=60 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
# modelos
l_md <- list()
l_md[[1]] <- glm(log_TE ~ M0 + I(M0^2),data = df_1sp,Gamma(log))
l_md[[2]] <- glm(log_TE ~ M0,data = df_1sp,Gamma(log))
l_md[[3]] <- glm(log_TE ~ log_M0 + I(log_M0^2),data = df_1sp,Gamma(log))
l_md[[4]] <- glm(log_TE ~ log_M0,data = df_1sp,Gamma(log))
l_md[[5]] <- glm(log_TE ~ 1,data = df_1sp,Gamma(log))
l_md[[6]] <- lm(log_TE ~ M0 + I(M0^2),data = df_1sp)
l_md[[7]] <- lm(log_TE ~ log_M0 + I(log_M0^2),data = df_1sp)
names(l_md) <- c("M0^2","M0","log(M0)^2","log(M0)","1","N:M0^2","N:log(M0)^2")
AICctab(l_md,weights=TRUE)
```

```{r graficos diagnostico do GLM Gamma(log) referencia TE 1sp,include=TRUE,eval=TRUE}
res.Dharma <- DHARMa::simulateResiduals(l_md[["log(M0)^2"]],n = 250,refit = FALSE)
plot(res.Dharma)
```
...
```{r sumário A6 do LM referencia do tempo de extincao na ausencia da residente}
summary(l_md[["log(M0)^2"]])
```

```{r graficos diagnostico padrao lm TE 1sp}
par(mfrow=c(2,2))
plot(l_md[["log(M0)^2"]])
```

  
__Figura A6__ gráficos diagnostico do lm: log(TE) ~ log(M0)^2  
...
```{r lm log(TE) 1sp modelo medio}
# dados
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=100 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
# modelo referência
md_ref <- glm(TE ~ log_M0 + I(log_M0^2),data = df_1sp, Gamma(log), na.action = "na.fail")
# todas as combinações do modelo cheio
registerDoMC(3)
l_dredge_nInvasions <- llply(dredge(md_ref,trace = FALSE,evaluate=FALSE), eval,.parallel = TRUE)
# Model Averaging for the observed data
...
df_md <- df_1sp
# interval = 'confidence' ainda não é usado, apenas para uma avaliação visual
df_md$TE_predict <- predict(mdAvg_MuMIn,type="response")
# Gráfico
colors <- c("loess" = "blue", "1 : 1" = "black")
ggplot(df_md,aes(x=TE,y=TE_predict)) +
  geom_point(alpha=0.3) + 
  geom_smooth(aes(color="loess"),method = "loess",formula = "y~x",se=FALSE) +
  geom_abline(aes(color="1 : 1"),slope=1,intercept = 0,size=2) +
  labs(x="observado",
       y="predito",
       title="Gamma(log)[TE ~ log(M0)^2] Predito pelo modelo médio X Observado",
       color = "") +
  scale_color_manual(values = colors)
```
  
    
__Figura A7__  predito e observado pelo modelo médio: lm - log(TE) ~ log(M0)^2  
  
    
##### Predição final Modelo Médio LM: log(TE) ~ log(M0)^2  
  
```{r LM TE 1 sp - modelo medio para novo conjunto de dados,echo=FALSE,eval=FALSE}
# dados
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=50 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
# modelo referência
md_ref <- lm(log_TE ~ log_M0 + I(log_M0^2),data = df_1sp,na.action = "na.fail")
# todas as combinações do modelo cheio
registerDoMC(3)
l_dredge_nInvasions <- llply(dredge(md_ref,trace = FALSE,evaluate=FALSE), eval,.parallel = TRUE)
# Model Averaging for new data predictions
df_pred <- df_1sp
  # expand.grid(log_M0=seq(min(df_1sp$log_M0),max(df_1sp$log_M0),length=60))
# esse demora com esse tanto de pontos
df_AICcmodavg_TE1sp <- modavgPred(l_dredge_nInvasions,newdata = df_pred) %>%
  cbind(df_pred)
df_AICcmodavg_TE1sp <- df_AICcmodavg_TE1sp |> 
  select(mod.avg.pred,lower.CL,upper.CL,M0,TE,log_TE,log_M0) |> 
  mutate(TE_predict = exp(mod.avg.pred),
         TE_lower = exp(lower.CL),
         TE_upper = exp(upper.CL))
write_csv(df_AICcmodavg_TE1sp, file="resultados/df_AICcmodavg_TE1sp.csv")
```
```{r LM TE 1 sp - graficos exploratorios modelo medio,eval=TRUE,echo=FALSE,fig.width=8,fig.height=4}
df_AICcmodavg_TE1sp <- read_csv("resultados/df_AICcmodavg_TE1sp.csv")
l_p <- list()
l_p[[1]] <- df_AICcmodavg_TE1sp |> 
  mutate(label = "log(Tempo Extinção)") |> 
  ggplot(aes(x=M0,y=log_TE)) +
  geom_point() +
  geom_ribbon(aes(ymin=lower.CL,ymax=upper.CL),color="blue") +
  geom_line(aes(x=M0,y=mod.avg.pred),color="red",size=1,alpha=0.9) +
  labs(y="") +
  facet_wrap(~label)
l_p[[2]] <- df_AICcmodavg_TE1sp |> 
  mutate(label = "Tempo Extinção") |> 
  ggplot(aes(x=M0,y=TE)) +
  geom_point() +
  geom_ribbon(aes(ymin=TE_lower,ymax=TE_upper),color="blue") +
  geom_line(aes(x=M0,y=TE_predict),color="red",size=1,alpha=0.9) +
  labs(y="") +
  facet_wrap(~label)
grid.arrange(grobs=l_p,ncol=2,top="Tempo de extinção observado e predito pelo modelo médio - LM:log(TE) ~ log(M0)^2")    
```
  
    
__Figura A8__   Predito pelo modelo médio (lm: log(TE) ~ log(M0)) e o observado na escala log(TE) e na escala padrão TE. Em vermelho a média e em azul o intervalo de confiança computado pressupondo normalidade assintótica (REF Modavgpred)

  
##### Tempo de extinção na presença da residente
  
```{r graficos exploratorios TEI 2spp,echo=FALSE}
df_2sp <- read_csv(file="resultados/df_TEI2spp.csv") |> 
  mutate(log_TE = log(TE_m), log_M0 = log(M0)) |> 
  dplyr::rename(TE = TE_m)
l_p <- list()
l_p[[1]] <- df_2sp |> 
  pivot_longer(cols=c("TE","log_TE"),names_to = "variaveis",values_to = "valores") |> 
  ggplot(aes(x=,y=valores)) +
...
# modelos
l_md <- list()
l_md[[1]] <- lmer(log_TE ~ (C_nm.z + I(C_nm.z^2))*(M0.z + I(M0.z^2)) + (1|id),data = df_2sp)
l_md[[2]] <- lmer(log_TE ~ (C_nm.z + I(C_nm.z^2))+(M0.z + I(M0.z^2)) + (1|id),data = df_2sp)
l_md[[3]] <- lmer(log_TE ~ (M0.z + I(M0.z^2)) + (1|id),data = df_2sp)
l_md[[4]] <- lmer(log_TE ~ (C_nm.z + I(C_nm.z^2))*(log_M0.z + I(log_M0.z^2)) + (1|id),data = df_2sp)
l_md[[5]] <- lmer(log_TE ~ (C_nm.z + I(C_nm.z^2))+(log_M0.z + I(log_M0.z^2)) + (1|id),data = df_2sp)
l_md[[6]] <- lmer(log_TE ~ (log_M0.z + I(log_M0.z^2)) + (1|id),data = df_2sp)
l_md[[7]] <- lmer(log_TE ~ (C_nm.z + I(C_nm.z^2)) + (1|id),data = df_2sp)
l_md[[8]] <- lmer(log_TE ~ 1 + (1|id),data = df_2sp)
...
__Figura A10__ Resíduos quantílicos lmer TEI 2spp cheio: C_nm^2 + log(M0)^2


__Tabela A8__ sumário do lmer TEI 2spp cheio mais plausível: C_nm^2 + log(M0)^2  
