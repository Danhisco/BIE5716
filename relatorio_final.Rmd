...
```{r simulacao tempo de extincao das invasoras,eval=FALSE,echo=TRUE}
df_replicas <- read_csv("resultados/df_rep_tFinal.csv")
...
```
```{r análise dos dados da pasta single_species}
# df <- df_sim[1,]
...
O modelo cheio mais plausível é aquele que considera a interação entre os polinômios (Tabela 1). Porêm este modelo não apresenta um excelente ajuste aos dados (Figura 1 e sumário do módelo 1). O predito e observado diferem em valores extremos: quando . 
...
__Figura Observado e predito pelo modelo médio__

```{r glm binomial observado e predito modelo medio, comment=FALSE, message=FALSE,warning=FALSE,results="hide"}
# Predição
df_md <- df_resultados
