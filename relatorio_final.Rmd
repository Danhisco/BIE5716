...
df_sim <- read_csv("csv/df_sim0.csv") |> 
  ...
  mutate(path_original = path,
         path = gsub("md_Estocastico","md_Estocastico_ATUAL",path_original))
# X <- df_sim[74,]
...
registerDoMC(3)
df_rep.tFinal <- adply(df_sim[1:74,],1,f_resultados,.parallel = TRUE)
df_rep.tFinal |> filter(invasion==1 & M0==2 & C_nm==-0.5)
...
<!-- ### Tamanho populacion da residente nos eventos de ausência de     -->
