...
# library(trackdown)
# both overwrite: 
# override the current google drive file with the current Rmd:
update_file(file ="relatorio_final.Rmd",
            gpath = "mestrado/disciplinas/BIE5716/",
            hide_code = TRUE)
# override the current Rmd with the current google drive 
...
__Observado e predito pelo modelo médio__

```{r glm binomial observado e predito modelo medio, comment=FALSE, message=FALSE,warning=FALSE,results="hide",fig.width=8, fig.height=30}
# Predição
df_md <- df_resultados
...
ggplot(df_md,aes(x=c_invasion,y=Pr_invasion)) +
  ...
  labs(x="número de substituições",
       ...
       title="Predito X Observado",
       color = "") +
  scale_color_manual(values = colors)
```
  
  
__Predito pelo modelo médio para novo conjunto de dados__

```{r figura 225 predito modelo medio novo conjunto de dados}
# dados
load(file="resultados/df_AICcmodavg.csv")
df_plot <- df_AICcmodavg %>% 
  mutate(M0 = M0.z * sd(df_resultados$M0) + mean(df_resultados$M0),
         C_nm = C_nm.z*sd(df_resultados$C_nm) + mean(df_resultados$C_nm))
# graficos
max_fill <- max(c(df_plot$mod.avg.pred,df_resultados$c_invasion/100,
                  df_plot$lower.CL,df_plot$upper.CL)) + 0.04
l_p <- list()
l_p[[1]] <- ggplot(df_plot,aes(x=M0,y=C_nm,fill=mod.avg.pred)) +
  geom_tile() +
...
```{r f 1sppSLV, echo=TRUE,eval=FALSE}
f_1sppSLV <- function(N0,
                      beta=df_replicas$beta[1],
                      delta=df_replicas$delta[1],
                      gamma=df_replicas$gamma[1],
                      path_csv){
 df_ab <- data.frame(n=N0,t=0)
 v_t <- c(1,-1) # 2 transitions
 while(df_ab[nrow(df_ab),"n"]!=0){
   v_rates <- c(
     n_p = df_ab[nrow(df_ab),"n"] * beta,
     n_m = df_ab[nrow(df_ab),"n"] * (delta + (df_ab[nrow(df_ab),"n"]-1) * gamma/2)
     )
   v_Srates <- sum(v_rates)
   # update
   ...
   readr::write_csv(df_ab,file = path_csv)
}
```



### Como a presença da espécie residente reduz o tempo de extinção da invasora?  
  
  
```{r preparacao simulacao tempo de extincao das invasoras,eval=FALSE,echo=FALSE}
df_replicas <- read_csv("resultados/df_rep_tFinal.csv")
df_invasoras <- df_replicas |> 
  select(id,.n,m,t,invasion)
df_sim_invasora <- df_invasoras |> 
  filter(invasion == 1) |> 
  mutate(path_ = paste0("csv/md_Estocastico/single_specie/id",id,"_rep",.n,".csv"))
# testes:
beta = df_replicas$beta[1]
delta = df_replicas$delta[1]
gamma = df_replicas$gamma[1]
N0 = df_sim$m[1]
path_csv = df_sim$path_[1]
# simulação geral
a_ply(df_sim[1:2,],1,function(df) f_1sppSLV(N0 = df$m,path_csv = df$path_))
```
