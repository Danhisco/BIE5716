...
plyr::a_ply(df_sim2,function(df) f_2sppDLV(X=df, max_t = df$t_max_anos,int_t = 0.02))
...
```{r análises preparatórias 1 - prob de, eval=FALSE, echo=FALSE, include=FALSE}
...
```{r 2sppDLV, echo=TRUE,eval=FALSE,include=TRUE}
f_2sppDLV <- function(X,max_t,int_t){
 f_ode<- function(t,y,p){
   N <- y[1]
   M <- y[2]
