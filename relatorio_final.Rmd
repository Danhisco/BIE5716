...
download_file(file = "Suggested_Exercises.Rmd",
              gpath = "mestrado/disciplinas/BIE5716/")
```
...
Me pergunto como a estocasticidade demográfica pode modificar o esperado pela aproximação de campo médio nos cenários de invasão, com a intenção de avaliar se é possível ter alguma intuição sobre o comportamente do sistema a partir de uma aproximação que desconsidera a variabilidade no sistema. Ao desconsiderar a variabilidade do sistema estocastico, propriedades importantes do sistema são negligenciadas como a extinção de todas as espécies dado tempo suficiente.

Aqui vamos simulação até  Uma expectativa é que a probabilidade da espécie invasora substituir a espécie residente deve aumentar quanto maior o tamanho populacional inicial da invasora e maior a assimetria competitiva em favor da invasora. Pois, quanto menor o tamanho populacional maior a chance de uma sequência de eventos demográficos leve à extinção da invasora, mesmo quando dominante. E quanto maior a assimetria competitiva em favor da invasora, maior as chances da espécie residente reduzir em abundância.

, pressuposto que o tempo para exclusão na aproximão de campo médio é diretamente relacionado com o tempo de substituição no modelo estocastico, quanto maior o tempo para exclusão na aproximação, maior deve ser o número de eventos demográficos probabilisticos no modelo estocástico para ocorrer alguma extinção, e portanto maior a chance de 
até  que podem divergir 


estável previsto pela situação 

Avaliamos o aumento do tamanho populacional da invasora e a variação na assimetria competitiva entre as duas espécies. Descrevo as reações demográficas consideradas, a equação mestra da dinâmica ecológica e as aproximações de campo médio da equação mestra. Então comparo com o observado numericamente usando uma implementação do algortimo de Gillespie (1977).
