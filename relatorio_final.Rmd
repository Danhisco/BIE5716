...
df_1sp <- read_csv(file="resultados/df_tParcial.csv") |> 
  select(-M0,-(id:n),-(TE_res:t)) |> 
  rename(M0 = m) |> 
  mutate(TE = TE_1sp/365.25,
         log_TE = log(TE),
         log_M0 = log(M0))
l_p <- list()
l_p[[1]] <- df_1sp |> 
...
# modelos
l_md <- list()
l_md[[1]] <- glm(TE ~ M0 + I(M0^2) + I(M0^3),data = df_1sp,Gamma(log))
l_md[[2]] <- glm(TE ~ M0 + I(M0^2),data = df_1sp,Gamma(log))
l_md[[3]] <- glm(TE ~ M0,data = df_1sp,Gamma(log))
l_md[[4]] <- glm(TE ~ log_M0 + I(log_M0^2) + I(log_M0^3),data = df_1sp,Gamma(log))
l_md[[5]] <- glm(TE ~ log_M0 + I(log_M0^2),data = df_1sp,Gamma(log))
l_md[[6]] <- glm(TE ~ log_M0,data = df_1sp,Gamma(log))
l_md[[7]] <- glm(TE ~ 1,data = df_1sp,Gamma(log))
names(l_md) <- c("M0^3","M0^2","M0","log(M0)^3","log(M0)^2","log(M0)","1")
AICctab(l_md,weights=TRUE)
```
...
df_AICcmodavg <- read_csv("resultados/df_GLMGamma_modavg_1sp.csv")
l_p <- list()
l_p[[1]] <- df_AICcmodavg |> 
...
__Figura A6__ Observado e predito GLM Gamma(log)
