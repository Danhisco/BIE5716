...
```{r ajuste modelo cheio lm TE 1sp}
# dados
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=100 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
...
```{r graficos diagnostico do LM referencia TE 1sp,include=TRUE,eval=TRUE}
...
```{r sumário do LM referencia do tempo de extincao na ausencia da residente}
summary(l_md[["log(M0)^2"]])
```

```{r graficos diagnostico padrao}
par(mfrow=c(2,2))
plot(l_md[["log(M0)^2"]])
```
...
```
```{r LM TE 1 sp - graficos exploratorios modelo medio}
