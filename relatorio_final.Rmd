...
```
```{r analise dos dados de tempo de extincao conjunto M0 menor do que100, fig.height=5,fig.width=8}
df_1sp <- read_csv(file="resultados/df_TE1sp.csv") |> 
  filter(M0<=100 & M0>1) |> 
  mutate(log_TE = log(TE), log_M0 = log(M0))
l_p <- list()
...
l_p[[4]] <- df_1sp |> 
  ...
  geom_smooth(method = "loess",formula = "y~x") +
  facet_wrap(~label)
grid.arrange(grobs=l_p,ncol=2,top="Conjunto Parcial: 100 >= tamanho inicial > 1")
```
...
l_md[[3]] <- lm(log_TE ~ 1,data = df_1sp)
names(l_md) <- c("M0^2","M0","1")
AICctab(l_md,weights=TRUE)
```

```{r graficos diagnostico do LM referencia TE 1sp,include=TRUE,eval=TRUE}
res.Dharma <- DHARMa::simulateResiduals(l_md[["M0^2"]],n = 250,refit = FALSE)
plot(res.Dharma)
```
